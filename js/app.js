  ...
  function setTool(title, html, setup) {
  $$('.tools-list li').forEach(li => li.classList.remove('active'));
  $('#mainContent').innerHTML = `
        <div class="card">
          <div class="card-header">
            <div><h2 class="card-title">${title}</h2></div>
            <span class="badge">Client-side</span>
          </div>
          <div id="toolContent">${html}</div>
        </div>
      `;
  if (setup) { try { setup(); } catch (e) { toast('Error: ' + e.message, 'error'); } }
}
  ...
  setTool(name, `
        <div style="text-align:center;padding:60px 20px;">
          ...
          <h3 style="margin-bottom:12px;">Tool Placeholder</h3>
  ...
  $('#mainContent').innerHTML = `
        <div class="card">
          <div class="welcome-section">
            ...
            <h1 class="welcome-title">Utility Toolbox</h1>
            <p class="welcome-text">
              Your all-in-one collection of developer utilities. Everything runs client-side with modern web technologies.
            ...
            <div class="feature-grid">
              ...
              <div class="feature-card">
                ...
                <p>Instant results with no network delays. Works offline once loaded.</p>
              </div>
              <div class="feature-card">
                <h3>🎨 Modern UI</h3>
                <p>Beautiful, responsive design that works on all devices and screen sizes.</p>
              </div>
              <div class="feature-card">
                <h3>🔧 50+ Tools</h3>
                <p>Comprehensive collection of crypto, conversion, and development utilities.</p>
              </div>
            </div>
          </div>
        </div>
  ...
  const tools = [
  { id: 'token', name: 'Token generator', desc: 'Generate random string with the chars you want, uppercase or lowercase letters, numbers and/or symbols.', icon: 'fas fa-random' },
  { id: 'hash', name: 'Hash text', desc: 'Hash a text string using the function you need : MD5, SHA1, SHA256, SHA224, SHA512, SHA384, SHA3 or RIPEMD160', icon: 'fas fa-hashtag' },
  { id: 'bcrypt', name: 'Bcrypt', desc: 'Hash and compare text string using bcrypt. Bcrypt is a password-hashing function based on the Blowfish cipher.', icon: 'fas fa-lock' },
  { id: 'uuids', name: 'UUIDs generator', desc: 'A Universally Unique Identifier (UUID) is a 128-bit number used to identify information in computer systems. The number of possible UUIDs is 16^32, which is 2^128 or about 3.4x10^38 (which is a lot!).', icon: 'fas fa-fingerprint' },
  { id: 'ulid', name: 'ULID generator', desc: 'Generate random Universally Unique Lexicographically Sortable Identifier (ULID).', icon: 'fas fa-id-badge' },
  { id: 'encrypt', name: 'Encrypt / decrypt text', desc: 'Encrypt clear text and decrypt ciphertext using crypto algorithms like AES, TripleDES, Rabbit or RC4.', icon: 'fas fa-lock-open' },
  { id: 'bip39', name: 'BIP39 passphrase generator', desc: 'Generate a BIP39 passphrase from an existing or random mnemonic, or get the mnemonic from the passphrase.', icon: 'fas fa-coins' },
  { id: 'hmac', name: 'Hmac generator', desc: 'Computes a hash-based message authentication code (HMAC) using a secret key and your favorite hashing function.', icon: 'fas fa-signature' },
  { id: 'rsa', name: 'RSA key pair generator', desc: 'Generate a new random RSA private and public pem certificate key pair.', icon: 'fas fa-key' },
  { id: 'pwStrength', name: 'Password strength analyser', desc: 'Discover the strength of your password with this client-side-only password strength analyser and crack time estimation tool.', icon: 'fas fa-shield-check' },
  { id: 'pdfcheck', name: 'PDF signature checker', desc: 'Verify the signatures of a PDF file. A signed PDF file contains one or more signatures that may be used to determine whether the contents of the file have been altered since the file was signed.', icon: 'fas fa-file-signature' },
  { id: 'datetime', name: 'Date-time converter', desc: 'Convert date and time into the various different formats', icon: 'fas fa-calendar-alt' },
  { id: 'intbase', name: 'Integer base converter', desc: 'Convert a number between different bases (decimal, hexadecimal, binary, octal, base64, ...)', icon: 'fas fa-calculator' },
  { id: 'roman', name: 'Roman numeral converter', desc: 'Convert Roman numerals to numbers and convert numbers to Roman numerals.', icon: 'fas fa-hashtag' },
  { id: 'b64', name: 'Base64 string encoder/decoder', desc: 'Simply encode and decode strings into their base64 representation.', icon: 'fas fa-file-alt' },
  { id: 'b64file', name: 'Base64 file converter', desc: 'Convert a string, file, or image into its base64 representation.', icon: 'fas fa-file' },
  { id: 'color', name: 'Color converter', desc: 'Convert color between the different formats (hex, rgb, hsl and css name)', icon: 'fas fa-palette' },
  { id: 'case', name: 'Case converter', desc: 'Transform the case of a string and choose between different formats', icon: 'fas fa-text-height' },
  { id: 'nato', name: 'Text to NATO alphabet', desc: 'Transform text into the NATO phonetic alphabet for oral transmission.', icon: 'fas fa-broadcast-tower' },
  { id: 'asciibin', name: 'Text to ASCII binary', desc: 'Convert text to its ASCII binary representation and vice-versa.', icon: 'fas fa-binary' },
  { id: 'unicode', name: 'Text to Unicode', desc: 'Parse and convert text to unicode and vice-versa', icon: 'fas fa-globe-asia' },
  { id: 'yaml2json', name: 'YAML to JSON converter', desc: 'Simply convert YAML to JSON with this online live converter.', icon: 'fas fa-file-code' },
  { id: 'yaml2toml', name: 'YAML to TOML', desc: 'Parse and convert YAML to TOML.', icon: 'fas fa-file-code' },
  { id: 'json2yaml', name: 'JSON to YAML converter', desc: 'Simply convert JSON to YAML with this online live converter.', icon: 'fas fa-file-code' },
  { id: 'json2toml', name: 'JSON to TOML', desc: 'Parse and convert JSON to TOML.', icon: 'fas fa-file-code' },
  { id: 'listconv', name: 'List converter', desc: 'This tool can process column-based data and apply various changes (transpose, add prefix and suffix, reverse list, sort list, lowercase values, truncate values) to each row.', icon: 'fas fa-list' },
  { id: 'toml2json', name: 'TOML to JSON', desc: 'Parse and convert TOML to JSON.', icon: 'fas fa-file-code' },
  { id: 'toml2yaml', name: 'TOML to YAML', desc: 'Parse and convert TOML to YAML.', icon: 'fas fa-file-code' },
  { id: 'xml2json', name: 'XML to JSON', desc: 'Convert XML to JSON', icon: 'fas fa-file-code' },
  { id: 'json2xml', name: 'JSON to XML', desc: 'Convert JSON to XML', icon: 'fas fa-file-code' },
  { id: 'md2html', name: 'Markdown to HTML', desc: 'Convert Markdown to Html and allow to print (as PDF)', icon: 'fas fa-file-alt' },
  { id: 'urlenc', name: 'Encode/decode URL-formatted strings', desc: 'Encode text to URL-encoded format (also known as "percent-encoded"), or decode from it.', icon: 'fas fa-link' },
  { id: 'escapehtml', name: 'Escape HTML entities', desc: 'Escape or unescape HTML entities (replace characters like <,>, &, " and \' with their HTML version)', icon: 'fas fa-code' },
  { id: 'urlparser', name: 'URL parser', desc: 'Parse a URL into its separate constituent parts (protocol, origin, params, port, username-password, ...)', icon: 'fas fa-globe' },
  { id: 'deviceinfo', name: 'Device information', desc: 'Get information about your current device (screen size, pixel-ratio, user agent, ...)', icon: 'fas fa-mobile-alt' },
  { id: 'basicauth', name: 'Basic auth generator', desc: 'Generate a base64 basic auth header from a username and password.', icon: 'fas fa-user-shield' },
  { id: 'og', name: 'Open graph meta generator', desc: 'Generate open-graph and socials HTML meta tags for your website.', icon: 'fas fa-share-alt' },
  { id: 'otp', name: 'OTP code generator', desc: 'Generate and validate time-based OTP (one time password) for multi-factor authentication.', icon: 'fas fa-mobile-alt' },
  { id: 'mimetypes', name: 'MIME types', desc: 'Convert MIME types to file extensions and vice-versa.', icon: 'fas fa-file' },
  { id: 'jwt', name: 'JWT parser', desc: 'Parse and decode your JSON Web Token (jwt) and display its content.', icon: 'fas fa-key' },
  { id: 'keycode', name: 'Keycode info', desc: 'Find the javascript keycode, code, location and modifiers of any pressed key.', icon: 'fas fa-keyboard' },
  { id: 'slugify', name: 'Slugify string', desc: 'Make a string url, filename and id safe.', icon: 'fas fa-link' },
  { id: 'wysiwyg', name: 'HTML WYSIWYG editor', desc: 'Online, feature-rich WYSIWYG HTML editor which generates the source code of the content immediately.', icon: 'fas fa-edit' },
  { id: 'uap', name: 'User-agent parser', desc: 'Detect and parse Browser, Engine, OS, CPU, and Device type/model from an user-agent string.', icon: 'fas fa-user' },
  { id: 'httpcodes', name: 'HTTP status codes', desc: 'The list of all HTTP status codes, their name, and their meaning.', icon: 'fas fa-server' },
  { id: 'jsondiff', name: 'JSON diff', desc: 'Compare two JSON objects and get the differences between them.', icon: 'fas fa-exchange-alt' },
  { id: 'safelink', name: 'Outlook Safelink decoder', desc: 'Decode Outlook SafeLink links', icon: 'fas fa-link' },
  { id: 'gitcheat', name: 'Git cheatsheet', desc: 'Git is a decentralized version management software. With this cheatsheet, you will have quick access to the most common git commands.', icon: 'fab fa-git-alt' },
  { id: 'randport', name: 'Random port generator', desc: 'Generate random port numbers outside of the range of "known" ports (0-1023).', icon: 'fas fa-plug' },
  { id: 'crontab', name: 'Crontab generator', desc: 'Validate and generate crontab and get the human-readable description of the cron schedule.', icon: 'fas fa-clock' },
  { id: 'jsonpretty', name: 'JSON prettify and format', desc: 'Prettify your JSON string into a friendly, human-readable format.', icon: 'fas fa-file-code' },
  { id: 'jsonmin', name: 'JSON minify', desc: 'Minify and compress your JSON by removing unnecessary whitespace.', icon: 'fas fa-compress' },
  { id: 'jsontocsv', name: 'JSON to CSV', desc: 'Convert JSON to CSV with automatic header detection.', icon: 'fas fa-file-csv' },
  { id: 'sqlpretty', name: 'SQL prettify and format', desc: 'Format and prettify your SQL queries online (it supports various SQL dialects).', icon: 'fas fa-database' },
  { id: 'chmod', name: 'Chmod calculator', desc: 'Compute your chmod permissions and commands with this online chmod calculator.', icon: 'fas fa-user-cog' },
  { id: 'dockerrun', name: 'Docker run to Docker compose converter', desc: 'Transforms "docker run" commands into docker-compose files!', icon: 'fab fa-docker' },
  { id: 'xmlfmt', name: 'XML formatter', desc: 'Prettify your XML string into a friendly, human-readable format.', icon: 'fas fa-file-code' },
  { id: 'yamlpretty', name: 'YAML prettify and format', desc: 'Prettify your YAML string into a friendly, human-readable format.', icon: 'fas fa-file-code' },
  { id: 'emailnorm', name: 'Email normalizer', desc: 'Normalize email addresses to a standard format for easier comparison. Useful for deduplication and data cleaning.', icon: 'fas fa-envelope' },
  { id: 'regextester', name: 'Regex Tester', desc: 'Test your regular expressions with sample text.', icon: 'fas fa-search' },
  { id: 'regexcheat', name: 'Regex cheatsheet', desc: 'Javascript Regex/Regular Expression cheatsheet', icon: 'fas fa-search' },
  { id: 'ipv4sub', name: 'IPv4 subnet calculator', desc: 'Parse your IPv4 CIDR blocks and get all the info you need about your subnet.', icon: 'fas fa-network-wired' },
  { id: 'ipv4conv', name: 'IPv4 address converter', desc: 'Convert an IP address into decimal, binary, hexadecimal, or even an IPv6 representation of it.', icon: 'fas fa-exchange-alt' },
  { id: 'ipv4range', name: 'IPv4 range expander', desc: 'Given a start and an end IPv4 address, this tool calculates a valid IPv4 subnet along with its CIDR notation.', icon: 'fas fa-expand-arrows-alt' },
  { id: 'maclookup', name: 'MAC address lookup', desc: 'Find the vendor and manufacturer of a device by its MAC address.', icon: 'fas fa-ethernet' },
  { id: 'macgen', name: 'MAC address generator', desc: 'Enter the quantity and prefix. MAC addresses will be generated in your chosen case (uppercase or lowercase)', icon: 'fas fa-ethernet' },
  { id: 'ipv6ula', name: 'IPv6 ULA generator', desc: 'Generate your own local, non-routable IP addresses for your network according to RFC4193.', icon: 'fas fa-network-wired' },
  { id: 'matheval', name: 'Math evaluator', desc: 'A calculator for evaluating mathematical expressions. You can use functions like sqrt, cos, sin, abs, etc.', icon: 'fas fa-calculator' },
  { id: 'eta', name: 'ETA calculator', desc: 'An ETA (Estimated Time of Arrival) calculator to determine the approximate end time of a task, for example, the end time and duration of a file download.', icon: 'fas fa-clock' },
  { id: 'percent', name: 'Percentage calculator', desc: 'Easily calculate percentages from a value to another value, or from a percentage to a value.', icon: 'fas fa-percent' },
  { id: 'chronometer', name: 'Chronometer', desc: 'Monitor the duration of a thing. Basically a chronometer with simple chronometer features.', icon: 'fas fa-stopwatch' },
  { id: 'temp', name: 'Temperature converter', desc: 'Degrees temperature conversions for Kelvin, Celsius, Fahrenheit, Rankine, Delisle, Newton, Réaumur, and Rømer.', icon: 'fas fa-thermometer-half' },
  { id: 'benchmark', name: 'Benchmark builder', desc: 'Easily compare execution time of tasks with this very simple online benchmark builder.', icon: 'fas fa-chart-bar' },
  { id: 'lorem', name: 'Lorem ipsum generator', desc: 'Lorem ipsum is a placeholder text commonly used to demonstrate the visual form of a document or a typeface without relying on meaningful content', icon: 'fas fa-font' },
  { id: 'textstats', name: 'Text statistics', desc: 'Get information about a text, the number of characters, the number of words, its size in bytes, ...', icon: 'fas fa-chart-line' },
  { id: 'emoji', name: 'Emoji picker', desc: 'Copy and paste emojis easily and get the unicode and code points value of each emoji.', icon: 'fas fa-smile' },
  { id: 'obfuscator', name: 'String obfuscator', desc: 'Obfuscate a string (like a secret, an IBAN, or a token) to make it shareable and identifiable without revealing its content.', icon: 'fas fa-eye-slash' },
  { id: 'textdiff', name: 'Text diff', desc: 'Compare two texts and see the differences between them.', icon: 'fas fa-exchange-alt' },
  { id: 'numeronym', name: 'Numeronym generator', desc: 'A numeronym is a word where a number is used to form an abbreviation. For example, "i18n" is a numeronym of "internationalization" where 18 stands for the number of letters between the first i and the last n in the word.', icon: 'fas fa-hashtag' },
  { id: 'asciiart', name: 'ASCII Art Text Generator', desc: 'Generate ASCII art from text using various styles and fonts.', icon: 'fas fa-font' },
  { id: 'phoneparse', name: 'Phone parser and formatter', desc: 'Phone parser and formatter', icon: 'fas fa-phone' },
  { id: 'iban', name: 'IBAN validator and parser', desc: 'IBAN validator and parser', icon: 'fas fa-credit-card' },
  { id: 'qrcode', name: 'QR Code generator', desc: 'Generate and download a QR code for a URL (or just plain text), and customize the background and foreground colors.', icon: 'fas fa-qrcode' },
  { id: 'wifiqr', name: 'WiFi QR Code generator', desc: 'Generate and download QR codes for quick connections to WiFi networks.', icon: 'fas fa-wifi' },
  { id: 'svg', name: 'SVG placeholder generator', desc: 'Generate svg images to use as a placeholder in your applications.', icon: 'fas fa-vector-square' },
  { id: 'camera', name: 'Camera recorder', desc: 'Take a picture or record a video from your webcam or camera.', icon: 'fas fa-camera' }
  ];
  ...
  $('#mainContent').innerHTML = `
        <div class="card">
          <div class="welcome-section">
            ...
            <h1 class="welcome-title">All the Tools</h1>
            <p class="welcome-text">
              Your all-in-one collection of developer utilities. Everything runs client-side with modern web technologies.
  ...
  const func = window[toolId + 'Tool'];
  if (func) {
  func();
} else {
  placeholder(toolId.replace(/([A-Z])/g, ' $1').trim());
}
});
});
}
  // CRYPTO TOOLS
  function tokenTool() {
  setTool('Token Generator', `
        <div class="form-group"><label>Length</label><input id="len" type="number" value="32" min="8" max="256"/></div>
        <div class="btn-group"><button id="gen" class="btn">Generate</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
    $('#gen').addEventListener('click', () => {
      const n = Number($('#len').value) || 32;
      const arr = new Uint8Array(n);
      crypto.getRandomValues(arr);
      $('#out').textContent = Array.from(arr).map(b => b.toString(16).padStart(2, '0')).join('');
  ...
  function hashTool() {
  setTool('Hash Text', `
        <div class="form-group"><label>Algorithm</label><select id="alg"><option>SHA-1</option><option selected>SHA-256</option><option>SHA-384</option><option>SHA-512</option></select></div>
        <div class="form-group"><label>Text</label><textarea id="ht" rows="4"></textarea></div>
        <div class="btn-group"><button id="hbtn" class="btn">Hash</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="hout" class="output"></div>
      `, () => {
    $('#hbtn').addEventListener('click', async () => {
      const data = new TextEncoder().encode($('#ht').value);
      const d = await crypto.subtle.digest($('#alg').value, data);
      $('#hout').textContent = toHex(d);
      toast('Hash generated');
    });
    $('#copy').addEventListener('click', () => navigator.clipboard.writeText($('#hout').textContent).then(() => toast('Copied')));
  });
}
  ...
  function uuidsTool() {
  setTool('UUIDs Generator', `
        <div class="form-group"><label>Count</label><input id="count" type="number" value="5" min="1" max="100"/></div>
        <div class="btn-group"><button id="g" class="btn">Generate</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
  ...
  function ulidTool() {
  setTool('ULID Generator', `
        <div class="btn-group"><button id="g" class="btn">Generate</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
    const EN = '0123456789ABCDEFGHJKMNPQRSTVWXYZ';
    function encTime(ms, len) { let t = ms, s = ''; for (let i = 0; i < len; i++) { s = EN[t % 32] + s; t = Math.floor(t / 32); } return s; }
    function rand(len) { let r = ''; const a = new Uint8Array(len); crypto.getRandomValues(a); for (let i = 0; i < len; i++) r += EN[a[i] % 32]; return r; }
    $('#g').addEventListener('click', () => { $('#out').textContent = encTime(Date.now(), 10) + rand(16); toast('ULID generated'); });
    $('#copy').addEventListener('click', () => navigator.clipboard.writeText($('#out').textContent).then(() => toast('Copied')));
    $('#g').click();
  });
}
  ...
  setTool('Encrypt / Decrypt Text', `
        <div class="form-group"><label>Mode</label><select id="mode"><option>Encrypt</option><option>Decrypt</option></select></div>
        <div class="form-group"><label>Password</label><input id="pass" type="password"/></div>
        <div class="form-group"><label>Text</label><textarea id="text" rows="4"></textarea></div>
        <div class="btn-group"><button id="btn" class="btn">Process</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, async () => {
    ...
    $('#btn').addEventListener('click', async () => {
      ...
      if (!pass || !text) return toast('Enter password and text', 'error');
      const enc = new TextEncoder();
      ...
      if (mode === 'Encrypt') {
        ...
        const combined = new Uint8Array(salt.length + iv.length + result.byteLength);
        combined.set(salt, 0);
        combined.set(iv, salt.length);
        combined.set(new Uint8Array(result), salt.length + iv.length);
        $('#out').textContent = btoa(String.fromCharCode(...combined));
      } else {
        try {
          const combined = Uint8Array.from(atob(text), c => c.charCodeAt(0));
          const s = combined.slice(0,16);
          const i = combined.slice(16,28);
          ...
          $('#out').textContent = new TextDecoder().decode(result);
        } catch (e) {
  ...
  function hmacTool() {
  setTool('HMAC Generator', `
        <div class="form-group"><label>Algorithm</label><select id="alg"><option>SHA-1</option><option selected>SHA-256</option><option>SHA-384</option><option>SHA-512</option></select></div>
        <div class="form-group"><label>Key</label><input id="key" type="password" placeholder="Secret key"/></div>
        <div class="form-group"><label>Message</label><textarea id="msg" rows="3"></textarea></div>
        <div class="btn-group"><button id="btn" class="btn">Generate HMAC</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, async () => {
    $('#btn').addEventListener('click', async () => {
      const alg = $('#alg').value;
      ...
      const msg = $('#msg').value;
      if (!keyStr || !msg) return toast('Enter key and message', 'error');
      const enc = new TextEncoder();
      ...
      $('#out').textContent = toHex(signature);
      toast('HMAC generated');
    });
    $('#copy').addEventListener('click', () => navigator.clipboard.writeText($('#out').textContent).then(() => toast('Copied')));
  });
}
  ...
  function b64Tool() {
  setTool('Base64 Encoder/Decoder', `
        <div class="form-group"><label>Text</label><textarea id="text" rows="4" placeholder="Enter text to encode/decode"></textarea></div>
        <div class="form-group"><label>Mode</label><select id="mode"><option selected>Encode</option><option>Decode</option></select></div>
        <div class="btn-group"><button id="btn" class="btn">Process</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
    $('#btn').addEventListener('click', () => {
      ...
      if ($('#mode').value === 'Encode') {
        ...
        out = fromB64(text) || 'Invalid base64';
      }
      $('#out').textContent = out;
      toast('Processed');
    });
...
hobbies:
  ...
  setTool('YAML to TOML Converter', `
        <div class="row">
            <div class="form-group">
                <label>YAML Input</label>
                <textarea id="yaml" rows="8" placeholder="Enter YAML">name: John Doe
...
address:
  street: 123 Main St
  city: Anytown</textarea>
            </div>
            <div class="form-group">
                <label>TOML Output</label>
                <textarea id="toml" rows="8" placeholder="TOML will appear here" readonly></textarea>
            </div>
        </div>
        <div class="btn-group">
            <button id="convert" class="btn">Convert to TOML</button>
            <button id="copy" class="btn btn-secondary">Copy TOML</button>
    ...
    function jsonToToml(json, indent = 0, parentKey = '') {
      let toml = '';
      ...
      if (Array.isArray(json)) {
        ...
        json.forEach((item, index) => {
          if (typeof item === 'object' && item !== null) {
            toml += `\n${indentStr}[[${parentKey}]]\n`;
            toml += jsonToToml(item, indent + 1);
          } else {
            toml += `${indentStr}${JSON.stringify(item)},\n`;
          }
        });
        return toml;
      }
      ...
      for (const [key, value] of Object.entries(json)) {
        ...
        if (value === null) {
          toml += `${indentStr}${key} = null\n`;
        } else if (typeof value === 'boolean') {
          toml += `${indentStr}${key} = ${value}\n`;
          ...
          if (value.length === 0) {
            ...
            toml += `${indentStr}${key} = [${arrayStr}]\n`;
          }
        } else if (typeof value === 'object') {
          // Table
          toml += `\n${indentStr}[${fullKey}]\n`;
          toml += jsonToToml(value, indent + 1);
        }
      }
    ...
    $('#convert').addEventListener('click', () => {
      ...
      try {
        const jsonObj = yamlToJson(yamlText);
        const tomlText = jsonToToml(jsonObj);

        $('#toml').value = tomlText.trim();
        $('#error').style.display = 'none';
        toast('Conversion successful');
      } catch (error) {
        $('#error').textContent = `Error: ${error.message}`;
    ...
    $('#copy').addEventListener('click', () => {
      navigator.clipboard.writeText($('#toml').value).then(() => toast('TOML copied'));
    });
...
features:
  ...
  setTool('JSON to TOML Converter', `
        <div class="row">
            <div class="form-group">
                <label>JSON Input</label>
                <textarea id="json" rows="8" placeholder='{
  "name": "John Doe",
  "age": 30,
  "hobbies": ["reading", "hiking", "coding"]
        ...
        <div id="error" style="color: var(--error); margin-top: 8px; display: none;"></div>
    `, () => {
    // Reuse the jsonToToml function from above
    function jsonToToml(json, indent = 0, parentKey = '') {
      let toml = '';
      const indentStr = '  '.repeat(indent);
...
fig, grape</textarea>
  ...
  setTool('TOML to JSON Converter', `
        <div class="row">
            <div class="form-group">
                <label>TOML Input</label>
                <textarea id="toml" rows="8" placeholder="Enter TOML">name = "John Doe"
age = 30
hobbies = ["reading", "hiking", "coding"]
...
city = "Anytown"</textarea>
        ...
        <div class="btn-group">
            <button id="convert" class="btn">Convert to JSON</button>
            <button id="copy" class="btn btn-secondary">Copy JSON</button>
            <button id="example" class="btn btn-secondary">Load Example</button>
        </div>
        <div id="error" style="color: var(--error); margin-top: 8px; display: none;"></div>
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (inMultilineString) {
          if (line.endsWith('"""')) {
            ...
            multilineContent += line.slice(0, -3);
            setValue(result, currentPath.concat(multilineKey), multilineContent);
            multilineContent = '';
            multilineKey = '';
          } else {
        ...
        if (line.includes('"""')) {
          ...
          if (match) {
            ...
            multilineContent = line.slice(match[0].length - 3);
            continue;
          }
        }
        ...
        if (line.startsWith('[') && line.endsWith(']')) {
          const tablePath = line.slice(1, -1).split('.').map(s => s.trim());
        ...
        // Handle array of tables
        if (line.startsWith('[[') && line.endsWith(']]')) {
          const tablePath = line.slice(2, -2).split('.').map(s => s.trim());
          currentPath = tablePath;
          ensureArrayPath(result, currentPath);
          continue;
        }
        ...
        if (equalIndex !== -1) {
          ...
          if (value === 'true') parsedValue = true;
            ...
            parsedValue = value.slice(1, -1).replace(/\\"/g, '"').replace(/\\\\/g, '\\');
          } else if (value.startsWith('[') && value.endsWith(']')) {
            try {
              // Simple array parsing
              const arrayStr = value.slice(1, -1);
              const items = arrayStr.split(',').map(item => item.trim());
          ...
          setValue(result, currentPath.concat(key), parsedValue);
        }
      }
    ...
    function ensureArrayPath(obj, path) {
      let current = obj;
      for (let i = 0; i < path.length; i++) {
        if (!current[path[i]]) {
          current[path[i]] = [];
      ...
      if (!Array.isArray(current)) {
        const temp = current;
        current = [temp];
        setValue(obj, path.slice(0, -1), current);
      }
      current.push({});
    ...
    $('#convert').addEventListener('click', () => {
      const tomlText = $('#toml').value;
      const pretty = $('#pretty').checked;

      if (!tomlText.trim()) {
        toast('Please enter TOML', 'error');
        return;
      }

      try {
        const jsonObj = tomlToJson(tomlText);
    ...
    $('#copy').addEventListener('click', () => {
      navigator.clipboard.writeText($('#json').value).then(() => toast('JSON copied'));
    });
...
role = "backend"`;
  ...
  setTool('TOML to YAML Converter', `
        <div class="row">
            <div class="form-group">
                <label>TOML Input</label>
                <textarea id="toml" rows="8" placeholder="Enter TOML">name = "John Doe"
...
city = "Anytown"</textarea>
        ...
        <div class="form-group">
            ...
            <div style="display: flex; gap: 16px;">
                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="checkbox" id="indent" checked />
                    <span>Use 2-space indentation</span>
                </label>
            </div>
        </div>
        <div class="btn-group">
            <button id="convert" class="btn">Convert to YAML</button>
            <button id="copy" class="btn btn-secondary">Copy YAML</button>
            <button id="example" class="btn btn-secondary">Load Example</button>
        </div>
        <div id="error" style="color: var(--error); margin-top: 8px; display: none;"></div>
    ...
    function jsonToYaml(obj, indent = 2, useIndent = true) {
      const spaces = useIndent ? indent : 0;

      function convert(value, depth) {
        const indentStr = ' '.repeat(depth * spaces);
        ...
        if (Array.isArray(value)) {
          ...
          const simpleArray = value.every(item =>
            typeof item === 'string' ||
            typeof item === 'number' ||
            typeof item === 'boolean' ||
            item === null
          ...
          if (simpleArray) {
            const items = value.map(item => convert(item, 0));
            return `[${items.join(', ')}]`;
          }
    ...
    $('#convert').addEventListener('click', () => {
      const tomlText = $('#toml').value;
      const useIndent = $('#indent').checked;
      ...
      try {
        const jsonObj = tomlToJson(tomlText);
        const yamlText = jsonToYaml(jsonObj, 2, useIndent);
    ...
    $('#copy').addEventListener('click', () => {
      navigator.clipboard.writeText($('#yaml').value).then(() => toast('YAML copied'));
    });
...
[Link](https://example.com)"></textarea>
  ...
  setTool('Device Information', `
        <div class="btn-group"><button id="refresh" class="btn">Refresh</button></div>
        <div id="out" class="output"></div>
      `, () => {
    ...
    $('#refresh').addEventListener('click', getDeviceInfo);
    getDeviceInfo();
  });
}
  ...
  setTool('OTP Code Generator', `
        <div class="row">
            <div class="form-group">
                ...
                <input type="text" id="secret" placeholder="Enter base32 secret" value="JBSWY3DPEHPK3PXP" />
                <button id="generateSecret" class="btn btn-secondary" style="margin-top: 8px; width: 100%;">Generate Random Secret</button>
            </div>
            <div class="form-group">
                <label>Digits</label>
                <select id="digits">
        ...
        <div class="row">
            ...
            <div class="form-group">
                ...
                <input type="number" id="step" value="30" min="10" max="120" />
            </div>
        </div>
        <div class="row">
            <div class="form-group">
                <label>Current OTP</label>
                <input type="text" id="currentOTP" readonly style="font-size: 24px; text-align: center; font-weight: bold; letter-spacing: 4px;" />
            </div>
            <div class="form-group">
                <label>Time Remaining</label>
                <div id="timeRemaining" style="font-size: 18px; text-align: center; font-weight: bold;"></div>
            </div>
        </div>
        <div class="btn-group">
            <button id="generateOTP" class="btn">Generate OTP</button>
            <button id="copyOTP" class="btn btn-secondary">Copy OTP</button>
            <button id="validateOTP" class="btn btn-secondary">Validate OTP</button>
        </div>
        <div id="results" class="output"></div>
        <div style="margin-top: 16px; padding: 16px; background: var(--bg-secondary); border-radius: 8px;">
            <h4 style="margin-bottom: 8px;">📱 OTP Information</h4>
            <div style="font-size: 14px;">
                <p><strong>TOTP (Time-based One-Time Password)</strong> is commonly used for two-factor authentication.</p>
                <p><strong>QR Code Format:</strong> otpauth://totp/Example:user@example.com?secret=JBSWY3DPEHPK3PXP&issuer=Example</p>
            </div>
        </div>
    `, () => {
    let timerInterval = null;
    let currentSecret = 'JBSWY3DPEHPK3PXP';
    ...
    const generateTOTP = (secret, time, digits = 6, algorithm = 'SHA-1') => {
      try {
        // Convert secret to bytes
        const key = base32ToBytes(secret);

        // Calculate time steps
        const step = parseInt($('#step').value) || 30;
        const timeSteps = Math.floor(time / step);

        // Convert time to 8-byte array (big-endian)
        const timeBytes = new ArrayBuffer(8);
        const timeView = new DataView(timeBytes);
        timeView.setUint32(0, Math.floor(timeSteps / 0x100000000));
        timeView.setUint32(4, timeSteps & 0xFFFFFFFF);

        // Import key for HMAC
        const encoder = new TextEncoder();
        const keyData = encoder.encode(String.fromCharCode(...key));

        // Generate HMAC (simplified - in production use proper crypto)
        let hash;
        if (algorithm === 'SHA-1') {
          // Simple SHA-1 simulation
          const combined = new Uint8Array(keyData.length + timeBytes.byteLength);
          combined.set(keyData);
          combined.set(new Uint8Array(timeBytes), keyData.length);

          // Simple hash - in production use crypto.subtle.digest
          const simpleHash = new Uint8Array(20);
          for (let i = 0; i < combined.length; i++) {
            simpleHash[i % 20] ^= combined[i];
          }
          hash = simpleHash;
        } else {
    ...
    function updateOTP() {
      const secret = $('#secret').value.trim() || currentSecret;
      const digits = parseInt($('#digits').value);
      const algorithm = $('#algorithm').value;
      const step = parseInt($('#step').value);

      const now = Date.now() / 1000; // Current time in seconds
      const otp = generateTOTP(secret, now, digits, algorithm);

      $('#currentOTP').value = otp;

      // Update time remaining
      const timeLeft = step - (now % step);
      $('#timeRemaining').textContent = `${Math.floor(timeLeft)}s`;
      $('#timeRemaining').style.color = timeLeft < 10 ? 'var(--error)' : 'var(--success)';
    ...
    function validateOTP() {
      ...
      if (!secret) {
        toast('Please enter a secret key', 'error');
        return;
      }

      // Check current and previous/next intervals for validation
      const validOTPs = [];
      for (let i = -1; i <= 1; i++) {
        const time = now + (i * step);
        const otp = generateTOTP(secret, time, digits, algorithm);
        validOTPs.push(otp);
      }
      ...
      const results = [
        ...
        `Time Step: ${step}s`,
        '',
        '=== VALID OTPs (current ±1 interval) ===',
        `Previous: ${validOTPs[0]}`,
        `Current: ${validOTPs[1]}`,
        `Next: ${validOTPs[2]}`,
        '',
        '=== CURRENT TIME ===',
        new Date().toLocaleString(),
        `Unix timestamp: ${Math.floor(now)}`
      ].join('\n');

      $('#results').textContent = results;
      toast('OTP validation complete');
    }

    // Event listeners
    $('#generateSecret').addEventListener('click', generateRandomSecret);
    ...
    $('#copyOTP').addEventListener('click', () => {
      navigator.clipboard.writeText($('#currentOTP').value).then(() => toast('OTP copied'));
    });

    $('#validateOTP').addEventListener('click', validateOTP);
  ...
  function randportTool() {
  setTool('Random Port Generator', `
        <div class="form-group"><label>Count</label><input id="count" type="number" value="1" min="1" max="10"/></div>
        <div class="btn-group"><button id="gen" class="btn">Generate</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
    function randPort() {
    ...
    $('#gen').addEventListener('click', () => {
      const count = Number($('#count').value) || 1;
      const ports = Array.from({length: count}, randPort);
      $('#out').textContent = ports.join('\n');
      toast(`${count} port(s) generated`);
    });
    $('#copy').addEventListener('click', () => navigator.clipboard.writeText($('#out').textContent).then(() => toast('Copied')));
    $('#gen').click();
  });
  ...
  setTool('JSON Prettify', `
        <div class="form-group"><label>JSON</label><textarea id="json" rows="6" placeholder='{"key": "value"}'></textarea></div>
        <div class="btn-group"><button id="pretty" class="btn">Prettify</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
  ...
  function jsonminTool() {
  setTool('JSON Minify', `
        <div class="form-group"><label>JSON</label><textarea id="json" rows="6" placeholder='{"key": "value"}'></textarea></div>
        <div class="btn-group"><button id="min" class="btn">Minify</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
...
john+label@gmail.com"></textarea>
  ...
  setTool('MAC Address Generator', `
    ...
    $('#gen').addEventListener('click', () => {
      const count = Number($('#count').value) || 1;
      const macs = Array.from({length: count}, genMac);
      $('#out').textContent = macs.join('\n');
      toast(`${count} MAC(s) generated`);
    });
    $('#copy').addEventListener('click', () => navigator.clipboard.writeText($('#out').textContent).then(() => toast('Copied')));
    $('#gen').click();
  });
  ...
  // MATH TOOLS
  function mathevalTool() {
  setTool('Math Evaluator', `
        <div class="form-group"><label>Expression</label><input id="expr" type="text" value="2+2*3" /></div>
        <div class="btn-group"><button id="eval" class="btn">Evaluate</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
    $('#eval').addEventListener('click', () => {
      try {
        const res = Function('"use strict"; return (' + $('#expr').value + ')')();
        $('#out').textContent = res.toString();
        toast('Evaluated');
      } catch (e) {
  ...
  setTool('Percentage Calculator', `
        <div class="row">
          <div class="form-group"><label>Value</label><input id="val" type="number" value="25" /></div>
          <div class="form-group"><label>Total</label><input id="total" type="number" value="100" /></div>
        </div>
        <div class="btn-group"><button id="calc" class="btn">Calculate</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
    $('#calc').addEventListener('click', () => {
      const val = Number($('#val').value) || 0;
      const total = Number($('#total').value) || 1;
      const pct = ((val / total) * 100).toFixed(2);
      $('#out').textContent = `${val} is ${pct}% of ${total}`;
      toast('Calculated');
    });
    $('#copy').addEventListener('click', () => navigator.clipboard.writeText($('#out').textContent).then(() => toast('Copied')));
    $('#calc').click();
...
arr.forEach((_, i) => {
  ...
  setTool('Lorem Ipsum Generator', `
        <div class="form-group"><label>Paragraphs</label><input id="paras" type="number" value="3" min="1" max="20"/></div>
        <div class="btn-group"><button id="gen" class="btn">Generate</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
    const lorem = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.';
    $('#gen').addEventListener('click', () => {
      const count = Number($('#paras').value) || 1;
  ...
  setTool('Text Statistics', `
        <div class="form-group"><label>Text</label><textarea id="text" rows="6"></textarea></div>
        <div class="btn-group"><button id="stats" class="btn">Analyze</button><button id="copy" class="btn btn-secondary">Copy</button></div>
        <div id="out" class="output"></div>
      `, () => {
  ...
  function svgTool() {
  setTool('SVG Placeholder Generator', `
        <div class="row">
          <div class="form-group"><label>Width</label><input id="w" type="number" value="300" /></div>
          <div class="form-group"><label>Height</label><input id="h" type="number" value="150" /></div>
          <div class="form-group"><label>Text</label><input id="text" value="Placeholder" /></div>
        </div>
        <div class="btn-group"><button id="gen" class="btn">Generate</button><button id="copy" class="btn btn-secondary">Copy SVG</button></div>
        <div id="out" class="output" style="font-family: sans-serif; white-space: pre;"></div>
      `, () => {
    $('#gen').addEventListener('click', () => {
      const w = $('#w').value || 300;
      const h = $('#h').value || 150;
      const text = $('#text').value || 'Placeholder';
      const svg = `<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
            <rect width="100%" height="100%" fill="#${Math.floor(Math.random()*16777215).toString(16)}"/>
            <text x="50%" y="50%" text-anchor="middle" dy=".3em" font-size="16" fill="white">${text}</text>
          </svg>`;
      $('#out').textContent = svg;
      toast('Generated');
    });
  ...
  cat.addEventListener('click', () => {
    const list = $(`ul[data-list="${cat.dataset.category}"]`);
    list.classList.toggle('collapsed');
    cat.classList.toggle('collapsed');
  });
});

  $$('.tools-list li').forEach(li => {
  li.addEventListener('click', () => {
    const tool = li.dataset.tool;
    $$('.tools-list li').forEach(l => l.classList.remove('active'));
    li.classList.add('active');
    const func = window[tool + 'Tool'];
    if (func) {
      func();
    } else {
      placeholder(tool.replace(/([A-Z])/g, ' $1').trim());
    }
  });
});
  ...
  const visible = Array.from(lis).some(li => li.style.display !== 'none');
  cat.style.display = visible ? 'flex' : 'none';
  list.style.display = visible || q === '' ? 'block' : 'none';
});
});
